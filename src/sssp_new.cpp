...
class sssp_app : public IApplication<vertex_val_t, op_val_t, edge_t> {
public:
...
//        val.vval = padj->fid;
        ...
        if (num > 0) {
            ...
            if (changed) {
                for (unsigned int i = 0; i < num; i++) {
                    vertex_id oid = padj->edges[i].vid;
                    IApplication<vertex_val_t, op_val_t, edge_t>::send_msg(oid, val.vval+1);
    ...
    sssp_app< vertex_val_t, op_val_t, edge_t>() {

    }

    sssp_app< vertex_val_t, op_val_t, edge_t>(IWorker<vertex_val_t, op_val_t> *_pworker) : \
...
//template<typename vertex_val_t, typename op_val_t, typename edge_t>
//int sssp_app< vertex_val_t, op_val_t, edge_t >::msgc = 0;
//template<typename vertex_val_t, typename op_val_t, typename edge_t>
//int sssp_app< vertex_val_t, op_val_t, edge_t >::send_msgc = 0;
//
//template<typename vertex_val_t, typename op_val_t, typename edge_t>
//int sssp_app< vertex_val_t, op_val_t, edge_t >::updatec = 0;
...
int main(int argc, char *argv[]) {
    ...
    Engine<edge_t, vertex_val_t, op_val_t, \
            sssp_app< vertex_val_t, op_val_t, edge_t>> *engine = \
            new Engine<edge_t, vertex_val_t, op_val_t, \
            sssp_app< vertex_val_t, op_val_t, edge_t >> (500, gp, 1);


    engine->start();
    //logstream(LOG_DEBUG) << "msgc: " << sssp_app< vertex_val_t, op_val_t, edge_t >::msgc << std::endl; 
    delete engine;
}
